...
### 新增功能
- **终端连接稳定性增强**：WebSocket 心跳保活（ping/pong 25s 间隔），断线 5 秒自动重连
- **AI 命令复制/执行按钮**：AI 生成的命令块下方新增「复制」「执行」按钮，多行命令使用 bracketed paste 模式正确执行
- **软件 AI 安装助手**：自动分析系统环境（OS/架构/包管理器），AI 推荐可用版本，一键执行安装命令并实时 SSE 日志流输出
- **AI 一键应用**：Cron/MySQL/防火墙模块的 AI 生成结果支持一键填充到表单

### 历史版本
- **v1.1.1**：手机端适配优化、AI 浮窗一键应用、终端 AI 命令助手修复
- **v1.1.0**：AI 模块助手（MySQL/Cron/防火墙/Docker/终端/系统优化/仪表盘/Nginx）、SSE 流式输出、聊天持久化、全局 AI 浮窗
