...
| **终端 AI** | 任务描述 → Linux 命令，点击直接插入终端执行 | 终端侧边栏 |
...
- **多轮上下文**：在同一会话中持续对话优化配置，AI 记住之前的修改
...
- **AI 代码编辑助手**（对话修改代码、截图粘贴、Diff 对比）
- **AI 网页生成器**（描述需求自动生成网页、截图还原设计）
- **AI 安全检测**（智能检测木马/后门/恶意 JS/暗链/SEO 黑帽）
- 文件压缩 / 解压（zip、tar.gz）
- 文件搜索、WebShell 特征扫描

### 数据库管理
- MySQL 数据库 / 用户 / 权限管理（支持 8.0 / 8.4 / 9.1）
- **AI SQL 助手**（自然语言生成 SQL 查询语句）
- Redis 键值浏览、内存监控
...
### 其他功能
- FTP 用户管理
- 计划任务（Cron 可视化管理）+ **AI 定时任务生成**
- Web 终端（浏览器内 SSH）+ **AI 命令助手**（描述任务生成命令，点击插入终端）
- 系统优化（内核参数调优）+ **AI 优化分析**（自动采集参数，给出针对性建议）
- 面板自动更新
